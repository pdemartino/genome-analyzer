...
public abstract class WindowedSignalFilter extends SignalFilter {
   ...
   private static final double defWinSize = 1.;
   private static final double defStepSize = 1.;

   public Signal filter(Signal inputSignal) {
      // Retrieve window settings
      double winSize =  filterConfiguration.get("window") != null
              ? Double.valueOf((String)filterConfiguration.get("window")) : defWinSize;
      
      double stepSize =  filterConfiguration.get("step") != null
              ? Double.valueOf((String)filterConfiguration.get("step")) : defStepSize;
      
      
      logger.info(String.format("Applying sliding window (win: %s, step: %s)", winSize, stepSize));
      Signal windowedSignal = inputSignal.applySlidingWindow(winSize, stepSize);
      // get statistics
